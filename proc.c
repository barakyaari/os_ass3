...
userinit(void)
{
  ...
  p->cwd = namei("/");
  p->numOfPages = p->sz / PGSIZE;
  p->numOfPagesInFile = 0;
  p->state = RUNNABLE;
}
pte_t *selectPageToMove() {
  pte_t *pte;

  if ((pte = walkpgdir(proc->pgdir, (void *) 0, 0)) == 0) {
...
growproc(int n)
{
  ...
  if (n > 0) {
    int numOfPagesAfterAlloc = (sz + n) / PGSIZE;
